-- Round 4 follow-up (client): no double registration on the gold enquiry form
-- by IC. A normalized NRIC that already has an enquiry is rejected (P0009).
-- Enforced in submit_enquiry for NEW submissions only — prod already holds a
-- handful of legitimate historical duplicates, so a unique index would fail;
-- the in-function check leaves history untouched.
CREATE INDEX IF NOT EXISTS idx_enquiries_nric_norm ON enquiries(customer_nric_normalized);

-- Same 7-arg signature as 20260706000006 (no DROP needed); body identical plus
-- the duplicate-IC guard after NRIC normalization.
...
BEGIN
  ...
  -- One gold-form registration per IC.
  IF v_nric_norm <> '' AND EXISTS (
    SELECT 1 FROM enquiries WHERE customer_nric_normalized = v_nric_norm
  ) THEN
