...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!slot_id || !identifier) {
      return new Response(
        JSON.stringify({ error: 'missing_fields', message: 'slot_id and identifier (email or phone) are required' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    // Whether this event requires NRIC. Optional-NRIC events (nric_required=false)
    // let attendees who registered without an NRIC check out by email/phone alone.
    // Re-checked server-side so a crafted request can't drop the NRIC factor on an
    ...
    const { data: slotRegs } = await supabase
      .from('registrations')
      .select('id, status, invitee_phone, invitee_email, invitee_nric')
      .eq('slot_id', slot_id);
    ...
    // 1b. Cross-check second identifier (email or phone) among candidates
