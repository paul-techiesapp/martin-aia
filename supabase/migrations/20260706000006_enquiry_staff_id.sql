...
-- Recreate submit_enquiry with the extra optional parameter. Body copied
-- verbatim from the current authoritative definition
-- (20260702000003_enquiry_notify_all.sql), with three changes:
--   1. New trailing param p_staff_id text DEFAULT NULL (signature change, so
--      the old 6-arg overload must be dropped explicitly).
--   2. enquiries INSERT gains the staff_id column.
--   3. GRANT updated for the new 7-arg signature.
