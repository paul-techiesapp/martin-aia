-- Item 6 (feedback round 2): every enquiry must notify. Redefine submit_enquiry
-- identical to 20260630000003_submit_enquiry_roadtax.sql EXCEPT the final notify
-- is now UNCONDITIONAL (was `IF v_agent_id IS NOT NULL`). The edge function
-- decides recipients (tied agent when present + admin catch-all), so branch/house
-- enquiries with no tied agent still reach the admin.
