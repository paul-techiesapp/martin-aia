...
import { Landmark, Clock, CheckCircle2, XCircle, RotateCcw } from 'lucide-react';
import { RewardStatus } from '@agent-system/shared-types';
import {
  useMerchantSettlements,
  useSetMerchantSettlementStatus,
...
export function MerchantSettlementList() {
  const { toast } = useToast();
  const { data: rows, isLoading } = useMerchantSettlements();
  const setStatus = useSetMerchantSettlementStatus();

  const [statusFilter, setStatusFilter] = useState<string>('all');
  const [failTarget, setFailTarget] = useState<AdminSettlementRow | null>(null);
  ...
  const summary = useMemo(() => {
    const all = rows ?? [];
    const sum = (xs: AdminSettlementRow[]) => xs.reduce((s, r) => s + (Number(r.amount) || 0), 0);
  ...
  const runUpdate = async (id: string, status: RewardStatus, reason?: string) => {
    try {
      ...
      toast({ title: `Settlement ${verb}` });
    } catch (err: any) {
      toast({ title: 'Failed to update settlement', description: err.message, variant: 'error' });
    }
  };
  ...
  return (
    <div className="flex flex-col gap-4 animate-fade-in">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight text-foreground">Merchant Settlements</h1>
        <p className="text-sm text-muted-foreground">
          Payables RACC owes partner merchants. Each is created automatically when a vehicle renewal is confirmed.
        </p>
      </div>
      ...
      <StatCardGrid columns={4}>
        <StatCard
          ...
          subtitle={`${rows?.length ?? 0} settlement${(rows?.length ?? 0) === 1 ? '' : 's'}`}
          icon={Landmark}
      ...
      <Card>
        <CardHeader className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div className="space-y-1.5">
            <CardTitle>Settlement Payouts</CardTitle>
            <CardDescription>Mark settlements paid or failed and track when each was settled.</CardDescription>
          </div>
                  ...
                  {filtered.length === 0 ? (
                    ...
                    filtered.map((r) => {
                      ...
                      return (
                        <TableRow key={r.id}>
                          <TableCell>
                            <div className="text-sm font-medium text-foreground">{r.merchant?.name ?? '—'}</div>
                          </TableCell>
                          <TableCell>
      ...
      <Dialog open={!!failTarget} onOpenChange={(open) => !open && setFailTarget(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Mark Settlement as Failed</DialogTitle>
            <DialogDescription>
              Record why this settlement payout failed (e.g. invalid payout details). Shown to admins on the row.
            </DialogDescription>
          </DialogHeader>
